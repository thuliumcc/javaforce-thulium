...
*/
  ...
  public static class AVStream extends Structure {
    ...
    //} time_base;
    public long starttime, duration, nb_frames;
    public int disposition;
    public int discard;  //enum AVDiscard
    //struct AVRational {
    public int sample_aspect_ratio_num, sample_aspect_ratio_den;
    public Pointer metadata;
    //} sample_aspect_ratio;
    //struct AVRational {
    public int avg_frame_rate_num, avg_frame_rate_den;
    //} avg_frame_rate;
...
//    int avformat_alloc_output_context2(PointerByReference ref, AVOutputFormat of, String format, String filename);  //n/a in libav.org
    ...
    long av_stream_get_r_frame_rate(AVStream stream);  //returns num_den
    void av_stream_set_r_frame_rate(AVStream stream, long num_den);
  }
...
//        int unpadded_linesize = frame.nb_samples * avutil.av_get_bytes_per_sample(audio_codec_ctx.sample_fmt);
    ...
    public float getFrameRate() {
      if (video_codec_ctx == null) return 0;
/*
      JFLog.log("video_codec_ctx.time_base_num=" + video_codec_ctx.time_base_num);
      JFLog.log("video_codec_ctx.time_base_den=" + video_codec_ctx.time_base_den);
      JFLog.log("video_codec_ctx.ticks_per_frame=" + video_codec_ctx.ticks_per_frame);
      JFLog.log("video_stream.time_base_num=" + video_stream.time_base_num);
      JFLog.log("video_stream.time_base_den=" + video_stream.time_base_den);
      JFLog.log("video_stream.afr_num=" + video_stream.avg_frame_rate_num);
      JFLog.log("video_stream.afr_den=" + video_stream.avg_frame_rate_den);
      return video_codec_ctx.time_base_den / video_codec_ctx.time_base_num / video_codec_ctx.ticks_per_frame;
*/
      long value = avformat.av_stream_get_r_frame_rate(video_stream);
//      JFLog.log("r_frame_rate=" + Long.toString(value, 16));
      float num = (float)(value & 0x7fffffff);
      float den = (float)(value >>> 32);
      return num / den;
    }
